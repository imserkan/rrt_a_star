    ...
    for i in range(numNodes):
        ...
        if noCollision(random_point.getCoord(), near_point.getCoord(), obstacle_points):
            ...
            for k in range(heuristic_random_point):
                random_point_heuristic = Point([math.floor(np.random.rand() * x_max), math.floor(np.random.rand() * y_max)], 0, 0)
                ndist_heuristic = []
                for j in range(len(nodes)):
                    temp_heuristic = calculateDistance(nodes[j].getCoord(), random_point_heuristic.getCoord())
                    ndist_heuristic.append(temp_heuristic)
                minimum_index_heuristic = ndist_heuristic.index(min(ndist_heuristic))
                minimum_value_heuristic = min(ndist_heuristic)
                near_point_heuristic = nodes[minimum_index_heuristic]
                new_point_heuristic = steer(random_point_heuristic.getCoord(), near_point_heuristic.getCoord(), minimum_value_heuristic, eps)
                if noCollision(random_point_heuristic.getCoord(), near_point_heuristic.getCoord(), obstacle_points):
                    new_point_heuristic.setCost(calculateDistance(new_point_heuristic.getCoord(), near_point_heuristic.getCoord()) + near_point_heuristic.getCost())
                    if new_point.getCost() + calculateDistance(new_point.getCoord(), goal_point.getCoord()) > \
                            new_point_heuristic.getCost() + calculateDistance(new_point_heuristic.getCoord(), goal_point.getCoord()):
                        new_point = new_point_heuristic
                        near_point = near_point_heuristic
            plt.plot([new_point.getCoord()[0], near_point.getCoord()[0]],
                     [new_point.getCoord()[1], near_point.getCoord()[1]], color='blue')
